...
public class MouseController implements MouseListener, MouseMotionListener {
    ...
    public void mousePressed(MouseEvent e) {
        ...
        if (view.overViewContains(x, y)) {
            overViewOffsetX = x - view.getOverviewRect().getX();
            overViewOffsetY = y - view.getOverviewRect().getY();
        }

        if (view.markerContains(x, y)) {
            markerOffsetX = x - view.getMarker().getX();
            markerOffsetY = y - view.getMarker().getY();
        }
...
//        System.out.println(y);
        if (view.markerContains(x, y) && ((int) (view.getScale() * 10) == 10)) {
